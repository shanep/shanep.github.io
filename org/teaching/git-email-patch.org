...
#+TITLE: Email a Git Patch
#+subtitle: How to create and email a patch using git
#+Date: <2023-12-07 Thu>
...
#+caption: Fork the starter repository into your personal GitHub account
[[../images/fork-the-repo.png]]

#+caption: In the new fork name the repository with a unique name
[[../images/fork-name.png]]
...
#+caption: You now should have a new repository (forked from the starter template) that is ready to use.
[[../images/codespace-extensions.png]]

* Configure Git Email
:PROPERTIES:
:CUSTOM_ID: configure-git-email
:END:
...
Turn on [[https://support.google.com/accounts/answer/185839?hl=en&co=GENIE.Platform%3DDesktop][Two factor authentication]] for your Boise State provided email account.  Do NOT skip this
step. Boise State University uses Gmail as their email provider and Gmail requires you to use two
factor authentication in order to generate an app password.  You need to use your Boise State
University issued email account to send the email and you *must* have two factor authentication
turned on.
...
In order to use [[https://git-scm.com/docs/git-send-email][git send-email]] you will need to generate an app password. Navigate to
[[https://security.google.com/settings/security/apppasswords]] and generate a new app password. Make
sure and copy the password before you close the window because you will not be able to see it again.

#+caption: generate app password
[[../images/gen-app-password.png]]
...
1. Go back and ensure you have [[#turn-on-two-factor][two factor authentication]] enabled.
2. [[https://support.google.com/mail/answer/8154][Log out]] of your Gmail account
3. Log back into your Gmail account and make sure you did have to use two factor authentication
4. If the steps above fail then open an [[https://support.google.com/chrome/answer/95464][incognito tab]] and go back to the first step
...
#+caption:Open up a terminal in codespaces
[[../images/open-terminal.png]]

2. In the terminal type =git config --global --edit= and modify the file with the info listed
   below. You will need to change the info listed below to match your own name, email and *App
   Password* that you generated in the [[#generate-an-app-password][previous step]].
...
#+caption: Push your changes to your repository.
[[../images/git-sync-changes.png]]

* Create and email a patch file
:PROPERTIES:
:CUSTOM_ID: create-and-email-a-patch-file
:END:
...
[[../images/gmail-original-email.png]]

4. [@4] Copy the email to your clip board

#+caption: copy to clipboard
[[../images/gmail-copy-email.png]]
...
looked good you can now submit your patch for grading. The mailing list will be listed in your
canvas site.  When you submit to the mailing list you will automatically be cc'd on the email so you
will have a copy in your own email as proof that you completed the assignment. You are allowed to
submit up to 3 times without penalty.

Open a terminal and submit your patch.

#+begin_example
git checkout submit
git send-email --to example-mailing-list@example.com HEAD^
#+end_example

Assuming all went well you are now complete! You have created a patch file from a squash merge,
emailed it and tested the resulting patch. You are well on your way to becoming an advanced git
user!
